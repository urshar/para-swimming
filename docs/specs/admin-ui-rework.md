...
- **Titelzeile** (`flex items-center gap-2`): ein **Nur-Icon-Zurück-Button** links vom `<h1>` — `variant="primary"`
  (solides Akzent-Blau, damit er nicht übersehen wird — Erik), `icon="arrow-left"`, `size="sm"`, `title="Zurück"` +
...
Bearbeiten `filled` + `text-amber-500!`; Löschen `filled` + `text-red-500!`; sekundäre Navigation/Aktionen `filled`
+ `text-blue-500!`. Bereits bewusst gesetzte Sonderfarben blieben: PDF/Drucken lila (`text-purple-500!`),
Excel/MeetManager-Export grün (`text-emerald-500!`).
...
**Umfang** (8 Gruppen, je eigene Sichtprüfung + Tests): records · athletes · clubs · meets/championships (inkl.
`development`/`qualified`/`selection` mit View-Switcher) · qualifying-time-lists + base-times · wps · club-entries ·
cups/results/admin-documents/lenex/kleine Formulare · classifiers. Reine Titel+Untertitel-Seiten ohne
Back/Button-Leiste (nations/index, wps rankings/talent/clubs, cups/*-index, lenex review/import,
qualifying-excluded-disciplines) blieben unverändert.

**Nebenbefunde mitbehoben:**
