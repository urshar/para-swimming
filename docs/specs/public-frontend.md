...
|-------------|-------------------------|------------------------------------|
| Komponenten | Livewire Flux (Pro)     | **Tailkit** (HTML/Alpine-Variante) |
...
**Vor Phase 1 zu verifizieren:** ob Tailkits Tailwind-Konfiguration ohne Konflikt neben der bestehenden läuft, und ob
zwei Vite-Entries die Buildzeit vertretbar halten. Fällt das negativ aus, ist die Alternative ein gemeinsamer Entry mit
strikt getrennten Klassenpräfixen — deutlich unangenehmer, deshalb zuerst die saubere Variante prüfen.
...
- **Tailkit/Flux-Koexistenz** (§3.1) — vor Phase 1 zu verifizieren.
