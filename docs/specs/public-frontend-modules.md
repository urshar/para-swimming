...
**Vorab zu klären (blockierend):** Läuft Tailkits Tailwind-Konfiguration konfliktfrei neben der bestehenden, und bleibt
die Buildzeit mit zwei Entries vertretbar? Ergebnis dieser Prüfung entscheidet über den Rest der Phase.

**Tests** (`--group=public-p1`): Sprachweiterleitung, Cookie-Vorrang, `hreflang`, Document-Scopes, `/` führt nicht mehr
in den Login.
