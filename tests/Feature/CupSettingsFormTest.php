...
describe('store', function () {
    it('speichert gender_combined_group_ids und active_age_group_ids korrekt', function () {
        ...
        $this->actingAs(makeAdmin_cup13())
            ->post(route('cups.store'), cupPayload_cup13([
                ...
                'active_age_group_ids' => [$offen->id], // Jugend NICHT angehakt
            ]))
        ...
        expect(CupGroupSetting::where('cup_id', $cup->id)->where('sport_class_group_id', $group->id)->first()->gender_combined)
            ->toBeTrue()
            ->and($cup->isAgeGroupActive($jugend))->toBeFalse()
            ->and($cup->isAgeGroupActive($offen))->toBeTrue();
