...
@section('content')
    <div class="max-w-xl">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">LENEX Export</h1>
            <div class="mt-4">
        ...
        {{-- Rekord-Export ist eine eigene Seite (records.export, "Rekorde"-Nav-Bereich) — hier stand vorher
             zusätzlich eine zweite, deckungsgleiche Rekord-Export-Tab, die an dieselbe
             records.export.download-Route sendete. Konsolidiert: diese Seite exportiert nur noch Wettkämpfe. --}}
        <form method="POST" action="{{ route('lenex.export.download') }}">
            ...
            <div
                ...
                <flux:field>
                    ...
                    <flux:select variant="listbox" name="meet_id" required>
                        <flux:select.option value="" :selected="! $selectedMeetId">Bitte wählen…</flux:select.option>
                        @foreach($meets as $meet)
                            <flux:select.option value="{{ $meet->id }}"
                                                 :selected="(string) $selectedMeetId === (string) $meet->id">
