...
<head>
    ...
    <style>
        ...
        .subtitle { font-size: 11px; color: #555; margin: 0 0 20px 0; }
        h2 {
            font-size: 13px; margin: 18px 0 6px 0; padding: 6px 8px;
            background-color: #f0f0f0; border: 1px solid #ddd;
        }
        table { width: 100%; border-collapse: collapse; margin-bottom: 4px; }
        th, td { padding: 4px 8px; text-align: left; border-bottom: 1px solid #eee; }
        th { background-color: #fafafa; font-size: 10px; text-transform: uppercase; color: #666; }
        td.rank, th.rank { width: 40px; }
        td.rounds, th.rounds { width: 60px; text-align: right; }
        td.points, th.points { width: 90px; text-align: right; font-family: Courier, monospace; }
...
<p class="subtitle">
    ...
    @if($calculatedAt)
        · berechnet am {{ \Illuminate\Support\Carbon::parse($calculatedAt)->format('d.m.Y H:i') }} Uhr
    @endif
</p>
    ...
    <table>
        ...
        @foreach($bracket['results'] as $row)
            <tr>
                ...
                <td>{{ $row->club?->name }}</td>
                <td class="rounds">{{ $row->rounds_counted }}</td>
