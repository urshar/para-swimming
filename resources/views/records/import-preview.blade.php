    ...
    <div class="max-w-4xl">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-2">
                <flux:button href="{{ route('records.import') }}" variant="primary" icon="arrow-left" size="sm"
        ...
        <form method="POST" action="{{ route('records.import.run') }}"
              x-data="recordImportPreview()" data-config='@json($previewConfig)'>
            @csrf
            ...
            @if(count($preview['unknown_clubs']) > 0)
                <div
                    ...
                    <p class="text-xs text-zinc-400 mb-4">Diese Vereine wurden in der Datenbank nicht gefunden — ggf.
                        falsch geschrieben. Bitte entscheiden Sie für jeden Verein.</p>
                                    ...
                                    <span class="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                                        ...
                                        @if($ath['club_name'])
                                            <span class="text-zinc-400 text-xs"
                                                  data-club-key="{{ $ath['club_key'] }}"
                                                  data-lenex-name="{{ $ath['club_name'] }}"
                                                  x-text="'· ' + athleteClubName($el.dataset.clubKey, $el.dataset.lenexName)">· {{ $ath['club_name'] }}</span>
                                        @endif
